 ...
 * @returns
 */
export const extractDirectoryPathFromEntryPath = (
  itemPath: string
): string | undefined => {
  if (itemPath) {
 ...
 * @returns
 */
export const extractExtensionFromEntryPath = (
  itemPath: string
): string | undefined => {
  if (itemPath) {
    const lastExtensionIndex = itemPath.lastIndexOf(".");
    if (lastExtensionIndex > -1)
      return itemPath.substring(lastExtensionIndex + 1);
  }
  return undefined;
};
 ...
 */
export const extractLastFolderNameFromFolderPath = (
  folderPath: string
): string | undefined => {
  if (folderPath) {
    ...
    if (lastSeparatorIndex > -1)
      return folderPath.substring(lastSeparatorIndex + 1, lastCharacterIndex);
  }
  return undefined;
};
...
export const formatNumberAsDecimalNumber = (
  number?: number,
  decimalPlaces = 2,
  minimumFractionDigits = decimalPlaces,
  locale = numberFormatterDefaultLocale
): string => {
...
export const formatNumberAsStorageSize = (number?: number): string => {
  ...
  if (number < 1048576)
    return `${formatNumberAsDecimalNumber(number / 1024, 2, 0)} KB`;
  if (number < 1073741824)
    return `${formatNumberAsDecimalNumber(number / 1048576, 2, 0)} MB`;
  if (number < 1099511627776)
    return `${formatNumberAsDecimalNumber(number / 1073741824, 2, 0)} GB`;
  return `${formatNumberAsDecimalNumber(number / 1099511627776, 2, 0)} TB`;
};

export const formatNumberAsWholeNumber = (
  number?: number,
  locale = numberFormatterDefaultLocale
): string => {
  if (number === null || number === undefined) return "";
 ...
 */
export const establishVendorAccessToken = async (
  connectionItem: ConnectionItem,
  accountId: string,
  sessionAccessToken: string,
  vendorRefreshURI: string
): Promise<string> => {
  let accessToken;
  ...
  if (connectionItem.authorization![""].expires_at - Date.now() < 300000) {
    ...
    if (!response.ok) {
      ...
      throw new Error(
        "Unable to establish access token.|" + JSON.stringify(data)
      );
    }
